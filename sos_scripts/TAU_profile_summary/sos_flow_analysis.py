...
def sos_flow_analysis(run_dir):
    ...
    for n,r,h in zip(prog_names, prog_ranks, high_res):
        ...
        if h_str == "off":
            ...
            mpi_collective_bytes = get_group_counter(c,"Collective Bytes Sent",n_str,h_str)
            mpi_recv_bytes = get_group_counter(c,"MPI Receive Bytes",n_str,h_str)
            mpi_send_bytes = get_group_counter(c,"MPI Send Bytes",n_str,h_str)
            io_read_bytes = get_group_counter(c,"IO Bytes Read",n_str,h_str)
            ...
            mpi_send_bytes = get_group_counter(c,"Message size sent to all nodes",n_str,h_str)
